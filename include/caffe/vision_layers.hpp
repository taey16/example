...
class UnpoolingLayer : public Layer<Dtype> {
 public:
  explicit UnpoolingLayer(const LayerParameter& param) : Layer<Dtype>(param) {}
  ...
  virtual inline int ExactNumTopBlobs() const { return 1; }
  virtual inline int MinBottomBlobs() const { return 1; }
  // MAX UNPOOL layers can intput an extra bottom blob for the mask;
  // others can only input the pooled inputs.
